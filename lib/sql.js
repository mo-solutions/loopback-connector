...
SQLConnector.prototype.buildSelect = function(model, filter, options) {
  ...
  if (filter) {
    let relationsWhere = new ParameterizedSQL();

    if(filter.leftJoin) {
      let leftJoinWhere = this.buildJoin(selectStmt, "LEFT JOIN", filter.leftJoin, model, filter, options);
      relationsWhere.merge(leftJoinWhere);
    }
    
    if(filter.rightOuterJoin) {
      let rightOuterJoinWhere = this.buildJoin(selectStmt, "RIGHT OUTER JOIN", filter.rightOuterJoin, model, filter, options);
    ...
    if(filter.leftJoin || filter.rightOuterJoin) { // OR other joins
      ...
      let thisIdName = this.idName(model);
...
SQLConnector.prototype.buildJoin = function(selectStmt, joinType, join, model, filter, options) {
  ...
  for(var key in join) {
    if(join[key] && relations && relations[key]) {
      ...
      let realtion = relations[key];
      let relIdName = this.idName(realtion.model);
      let thisIdName = this.idName(model);
      ...
      let relWhereStmt = this.buildWhere(realtion.model, join[key]);
